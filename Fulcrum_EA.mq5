...
input int    InpP600             = 600;
input int    InpP2400            = 2400;
...
input double InpStopBufferATR    = 0.15;    // stop = 50 EMA value at entry +/- this many ATR
input double InpFixedTargetUSD   = 45.0;    // fixed $ target at InpLots=0.01 (scale with lot size)
input double InpMinStopATR       = 0.0;     // OPT-IN (0 = off = shipped v2.12 behaviour). Floor on the initial
...
input group "=== Misc ==="
input ulong  InpMagic            = 750091;
input string InpTradeComment     = "Fulcrum";
...
input string  InpWatermark     = "FULCRUM";          // empty = none
...
int OnInit()
  {
   ...
   if(h21==INVALID_HANDLE || h50==INVALID_HANDLE || h150==INVALID_HANDLE ||
     ...
     {
      Print("Fulcrum EA: failed to create an indicator handle");
      return(INIT_FAILED);
     }
   if(_Period != PERIOD_M5)
      Print("Fulcrum EA WARNING: validated on M5 - attach to an M5 chart.");
   ...
   if(InpLotMode != LOT_FIXED)
      Print("Fulcrum EA WARNING: InpLotMode is not LOT_FIXED - TargetDistance() will scale the "
      ...
      PrintFormat("Fulcrum EA WARNING: InpLots=%.2f, not the validated 0.01 - TargetDistance() will "
   ...
   if(!g_skipCosmeticDraws)
     {
      ...
      //--- levels would not appear until the next M5 bar closes, and on a
   ...
   if(tickVal <= 0.0 || tickSize <= 0.0)
     {
      ...
      PrintFormat("Fulcrum EA: TargetDistance() tick value/size read failed (tickVal=%.5f tickSize=%.5f) - "
...
void LogClosed(const string reason, const double exitPx, const double profit)
  {
   ...
   if(handle == INVALID_HANDLE)
     {
      Print("Fulcrum EA: failed to open CSV log, error ", GetLastError());
      return;
     }
   if(!exists)
   ...
   FileClose(handle);
   PrintFormat("Fulcrum EA: CSV export -> %s\\Files\\%s", TerminalInfoString(TERMINAL_COMMONDATA_PATH), fn);
  }
//+------------------------------------------------------------------+
   ...
   if(trade.PositionClose(g_ticket))
     {
      ...
      // silent.
      PrintFormat("Fulcrum EA: %s close FAILED for ticket %I64u, retcode %d (%s) - will retry next tick",
   ...
   if(sent)
     {
      ...
      g_posDir = isBuy ? 1 : -1;
      PrintFormat("Fulcrum EA: %s entry @ %.2f, stop %.2f, target %.2f",
                  (isBuy?"BUY":"SELL"), g_entryPx, stopPx, tgtPx);
     }
   else
      Print("Fulcrum EA: order send failed, error ", GetLastError());
  }
//+------------------------------------------------------------------+
...
void PBackground()
  {
   ...
   if(!ObjectCreate(0, nm, OBJ_BITMAP_LABEL, 0, 0, 0))
     { Print("Fulcrum EA BG: ObjectCreate failed, error ", GetLastError()); return; }
   ...
   if(!okSet || err != 0)
     {
      if(g_bgTries <= 3)
         PrintFormat("Fulcrum EA BG try %d: failed to load \"%s\" set=%s error=%d"
   ...
   PWatermark();
   PrintFormat("Fulcrum EA BG: loaded \"%s\" on try %d", path, g_bgTries);
   ChartRedraw(0);
  }
//+------------------------------------------------------------------+
void PWatermark()
...
void DrawPanel(const bool reclaim)
  {
   ...
   PText("t2", x + w - 12, ty + 3, "FULCRUM", InpTextCol, InpPanelSize, true);
   ty = y + hdr + 6;
