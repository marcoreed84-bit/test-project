...
//+------------------------------------------------------------------+
//|  v1.45 (2026-09-07) - M5 moving-average defaults changed, real MT5-        |
//|  confirmed: InpP600 600->500, InpM600 SMA->SMMA (supersedes v1.44's         |
//|  never-independently-confirmed EMA switch), InpP150 150->250 +              |
//|  InpM150 EMA->SMA, InpMinVolRatio 1.30->1.25. Found via a systematic         |
//|  Opus sweep (full MA period+method grid across both M5 and M15, plus         |
//|  applied-price and the other entry filters - SESSION_NOTES.md item 23)        |
//|  after the user asked directly whether a better MA stack existed rather        |
//|  than continuing to guess one combination at a time. Real MT5 backtest          |
//|  (2023.01-2026.09 GOLD M5, same period throughout): $913.35 (original            |
//|  shipped 600/SMA) -> $1,137.82 (600->500/SMMA alone) -> $1,599.72 (this           |
//|  update, all changes combined) - net +75.1%, PF 1.235->1.424 (+15.3%),             |
//|  Balance DD roughly flat (6.94%->8.08%), Equity DD up (7.11%->10.04%, the           |
//|  one real cost, not glossed over). Two real process lessons from getting             |
//|  here, worth remembering: (1) a real MT5 test that looked catastrophic on             |
//|  first read turned out to have InpM50/InpM150's MODE values swapped in the             |
//|  Tester (an easy mistake - both are numeric dropdowns stacked adjacently) -             |
//|  always read the actual input dump from a returned report before trusting               |
//|  its result, even when the filename says what was supposedly tested. (2) the             |
//|  Python screen that generated this candidate got the DIRECTION of the                     |
//|  Equity DD change wrong in an earlier isolated test of just the 600->500/SMMA              |
//|  change (predicted improvement, real result was worse) - Python screening in               |
//|  this system has repeatedly proven reliable for RANKING candidates against                  |
//|  each other but not for absolute risk numbers; every recommendation here was                 |
//|  real-MT5-confirmed before being adopted, never taken on the Python numbers                   |
//|  alone. NOTE: this timeframe (M5) still loses to M15 on every risk-adjusted                    |
//|  metric even after this fix - see Aurelius_M15_EA.mq5 (a separate file, same                    |
//|  logic, M15-native MA defaults) if drawdown/PF matters more than staying on                      |
//|  M5 specifically.                                                                                  |
//|                                                                                                      |
//|  v1.46 (2026-09-10): InpMinSlopeATR default 0.50->0.40, CANDIDATE UNDER TEST.                        |
//|  Never independently swept before. A Python sweep (gate_loosen_test.py, real                          |
//|  M5 data, everything else at shipped v1.45 defaults) found 0.40 a genuine                              |
//|  local optimum, beating 0.50 on every measure - net +14.9%, PF up, max                                  |
//|  drawdown down. Same sweep also tested (and rejected) three other loosening                             |
//|  ideas: removing the pullback-to-50 requirement entirely (more trades, net                               |
//|  roughly flat, PF/drawdown worse), loosening InpMaxSpreadPoints (zero effect                              |
//|  at any threshold - real spread never exceeds 60 on this data, so the filter                              |
//|  isn't currently binding), and loosening InpMaxSlopeATR (monotonically WORSE                              |
//|  at every step tested, left at 1.00). See InpMinSlopeATR's own comment and                                 |
//|  SESSION_NOTES.md for the full numbers. NEEDS A REAL MT5 BACKTEST before                                    |
//|  trusting this over 0.50 - Python-only so far.                                                               |
//|                                                                                                                 |
//|  v1.47: adds InpPublishPosition (default on) - broadcasts this EA's real     |
//|  position direction via a terminal global variable ("AURELIUS_POSDIR_M5_"+   |
//|  Symbol), purely so Vanguard_EA.mq5 (a separate EA, its own chart) can        |
...
//+------------------------------------------------------------------+
//|  v1.50: InpUseGivebackExit added (default OFF, Python-only so far -      |
...
//|  InpUseBreakeven above (real-tested, rejected): a blanket "cut anything      |
//|  that gives back to near-breakeven" rule was tested first and LOST real      |
//|  money on Meridian_EA.mq5 (-$2037, -61%) because most of the edge comes       |
//|  from the minority of giveback trades that recover into a large winner,       |
//|  and a blanket rule can't tell those apart from the ones that go on to         |
...
input int     InpP150  = 250;                  // MA 150->250 period (2026-09-07): a systematic Opus sweep found MODE_SMA beats MODE_EMA on this leg across the entire 200-350 range, not just one lucky point.
                                                ...
                                                // PF +15% over the original shipped config, at a real
                                                // cost: Equity DD rose from 7.11% to 10.04% (Balance DD
                                                // stayed roughly flat, 6.94%->8.08%). Also real-tested:
                                                // this timeframe (M5) still underperforms M15 on every
...
input ENUM_PBMA    InpPullbackMA = PB_50;      // Which MA the pullback must reach
...
input double  InpMinSlopeATR     = 0.40;       // Minimum slope (x ATR) - REAL-CONFIRMED (2026-09-23, live GOLD real MT5 test, full detail below).
                                                ...
                                                // $2,204.06->$2,532.49 (+14.9%), PF 1.655->1.703, max
                                                // drawdown $148.61->$137.28. Looser still (0.30-0.10) keeps
                                                // raising net profit but drawdown climbs back up and fold-1
                                                // consistency weakens (goes flat/negative) - 0.40 is a real
                                                // peak, not "looser is always better". Same sweep also
                                                // tested loosening InpMaxSlopeATR (1.00->higher/off) -
...
//--- cross-EA signal (see InpPublishPosition) - M5-specific name so
//--- Vanguard_EA.mq5's own M5 reader and Vanguard_M15_EA.mq5's M15
//--- reader (reading Aurelius_M15_EA.mq5's own, separately-named
//--- variable) can never cross-connect to the wrong timeframe pair.
...
int OnInit()
  {
   ...
   //--- "M5" is literal, not PERIOD_CURRENT-derived: this file always runs
   //--- on M5 (see this EA's own symbol/period conventions), and Vanguard's
   //--- reader needs a name it can compute without ever having queried
   //--- this chart's actual period.
   g_posDirGVarName = "AURELIUS_POSDIR_M5_" + _Symbol;
   ...
   if(Period() != PERIOD_M5)
      Print("WARNING: this EA was tested on M5. Current timeframe is ",
...
void DrawPanel(const bool haveLong, const bool haveShort, const bool reclaim)
  {
   ...
   const int ROWS = (haveLong || haveShort) ? 39 : 37, GAPS = 10;
   ...
   PRow("c9", x, ty, w, "to S/R level",
        ...
        !InpUseSRDist ? -1 : (sd >= InpMinSRDistATR ? 1 : 0)); ty += rh + 6;
...
void OnTick()
  {
   ...
   //--- tick-level giveback-to-breakeven exit (InpUseGivebackExit, v1.50
